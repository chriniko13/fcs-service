...
public class SampleDataGenerator {
    ...
    private static void generate() throws Exception {
        ...
        // Note: merge captures by date part.
        Map<LocalDate, List<FieldConditionCapture>> capturesByDate = new LinkedHashMap<>();
        for (FieldConditionCapture capture : captures) {

            Instant occurrenceAt = capture.getOccurrenceAt();
            LocalDate localDate = ZonedDateTime.ofInstant(occurrenceAt, ZoneOffset.UTC).toLocalDate();

            List<FieldConditionCapture> capturesWithSameDate = capturesByDate.get(localDate);

            if (capturesWithSameDate == null) {
                List<FieldConditionCapture> l = new LinkedList<>();
                l.add(capture);
                capturesByDate.put(localDate, l);
            } else {
                capturesWithSameDate.add(capture);
            }
        }

        MathProvider mathProvider = new MathProvider();

        Map<LocalDate, Double> mergedCapturesByDate = new LinkedHashMap<>();
        for (Map.Entry<LocalDate, List<FieldConditionCapture>> entry : capturesByDate.entrySet()) {

            List<FieldConditionCapture> conditionCaptures = entry.getValue();

            double avg = conditionCaptures.stream().mapToDouble(FieldConditionCapture::getVegetation).average().getAsDouble();

            mergedCapturesByDate.put(entry.getKey(), avg);
        }


        // Note: calculate statistics.
        DoubleSummaryStatistics summaryStatistics = mergedCapturesByDate
                .entrySet()
                .stream()
                .sorted((o1, o2) -> -o1.getKey().compareTo(o2.getKey()))
                .filter(fC -> {
                    LocalDate date = fC.getKey();
                    LocalDate nowLocalDate = LocalDate.now(Clock.system(NOW.getZone()));

                    long daysDiff = Duration.between(date.atStartOfDay(), nowLocalDate.atStartOfDay()).toDays();
